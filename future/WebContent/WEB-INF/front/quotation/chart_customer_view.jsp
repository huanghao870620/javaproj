...
<head lang="en">
	...
	<script type="text/javascript" src="<%=contextPath %>/res/js/front/quotation_room/customer_view.js"></script>
	<script type="text/javascript" src="<%=contextPath %>/res/js/front/quotation_room/quotation_customer_actual.js"></script>
	<script type="text/javascript" src="<%=contextPath %>/res/js/front/quotation_room/quotation_customer_interaction.js"></script>
...
<body>
<input id="view_id" type="hidden" name="view_id"/>
		        	...
		        	<span class="port-head">
		        		<c:if test="${roleId ne '5' }">
		        			<c:if test="${customer.CFileId == null }">
		          				<img src="<%=contextPath %>/res/images/default.png" width="45" height="40" height="40">
		        			</c:if>
		        			<c:if test="${customer.CFileId != null }">
		        		...
		        		<c:if test="${roleId eq '5' }">
		        			<img src="<%=contextPath %>/res/images/level${customer.levelId }.png" width="48" height="20">
		        		</c:if>
		        	</span>
		        	<em class="my-name">${customer.name }</em>
        ...
        <div class="chart-live clearfix">
			...
			<!--                 <p class="c-bg"><a href="javascript:;">专题房间</a></p> -->
				<ul class="clearfix c-bg">
				    ...
				    <li class="point-item active"><a href="#">磊丹观点</a></li>
				    <li class="trans-item"><a href="#">交易内参</a></li>
				    <li class="account-item"><a href="#">账户诊断</a></li>
				<!--                     <li><a href="#">财经日历</a></li> -->
				<!--                     <li><a href="#">一对一</a></li> -->
				<!--                     <li><a href="#">软件下载</a></li> -->
				<!--                     <li><a href="#">产品介绍</a></li> -->
...
<!--                             <span>声音提示<i class="voice-on"></i></span> -->
                ...
                <div class="live-view">
                    <p class="live-tex" style="height: 2%;">观点直播</p>
                    <div class="live-view-list">
						<div class="ld-history">
                        	<div class="history-date">
                                <ul id="msg_teacher_view">
                                 	<c:forEach var="view" items="${teacherViewDtos}">
                                 		<li id="teacherView_li${view.id}"  class="green" style="display: list-item;">
                                            <dl>
                                                ...
                                                <dd class="teach-tex">
                                                	${view.viewContent}
                                                	<c:if test="${!view.isReplied }">
                                                		<a viewId="${view.id}" teacherId="${view.teacherId}" questionType="1" href="javascript:;" class="ask-a">向老师提问</a>
                                                	</c:if>
                                                </dd>
                                                <dd class="teach-img">
                                                	<c:choose>
			...
			<div class="sp-notice clearfix">
                ...
                <p class="fl p-w">
					<c:choose>
						...
						<c:otherwise>
							【重点公告】 下周1V1在麦时为20:00——23:00 直播间麦时为23:00—2:00，白天不定时在线，有做单机会及建议随时在1V1里发布，大家如果有问题可留言。
						 </c:otherwise>
					</c:choose>
                </p>
            </div>
            <!--学生看到的聊天界面开始--><!--因为影响显示效果所以注释了；直接复制即可-->
            <div class="sp-chart" style=" height: 62%;overflow-y: auto;">
                <ul class="sp-title clearfix">
					...
					<li id="interaction_li" >
						<a href="javascript:;">
							<em id="interaction_count_id" class="c-tip">
								<c:choose>
									<c:when test="${count > 99}">    
										99+
									</c:when>
									<c:otherwise>
										${count }
                ...
                <div class="sp-title-con">
                    ...
                    <div class="sp-title-con01" style="display: none;"  id="msg_interaction_div">
                        <ul id="interaction_view" class="sp-com">
                          <c:forEach items="${messageDtoList}" var="messageDto" varStatus="status" >
								<li id="interaction_li_${messageDto.user.userId }_${messageDto.viewId}" class="clearfix chart-sep interaction_li_${messageDto.viewId}">
									...
									<c:if test="${question.replyList != null}">
										<c:forEach var="reply" varStatus="replyStatus" items="${question.replyList }">
											<div class="chart-new">
													...
													<li class="chart_g">
														<div class="main-c">
															...
															<p class="function">
																...
																<c:if test="${questionStatus.count == quesIndex && replyStatus.last}">
																	<a viewId="${messageDto.viewId}" teacherId="${messageDto.viewCreatorId}" questionType="2" href="javascript:;" class="ask-a">回复</a>
																</c:if>
															</p>
														</div>
													</li>
...
<!--弹窗开始-->
	<!--star向老师提问弹窗-->
	<div class="q-layout q-t-layout" style="display: none;">
		<i class="close-q"></i>
		<p class="q-title"><i class="q-title-icon"></i>向老师提问</p>
		<textarea id="askToTeacher_content" class="text-a"></textarea>
		<a href="javascript:customerToTeacher();" class="q-btn">提问</a>
	</div>
	<!--end向老师提问弹窗-->
    <!--star回复学生弹窗-->
    <div class="q-layout a-rep">
        ...
        <textarea class="text-a">我今天得策略都很精准。/握手</textarea>
        <a href="javascript:;" class="q-btn">提问</a>
    </div>
    <!--end回复学生弹窗-->
    <!--star点击图片效果-->
    <div class="img-b">
	...
	<div class="bg point-box" style="display: none;">
	    <div class="point position-middle">
	        <div class="point-con">
	            <!--开始-->
	            <div class="banner_view"><img src="<%=contextPath %>/res/images/banner_view.png" /></div>
	...
	<div class="bg trans-box" style="display: none;">
	    <div class="trans position-middle">
	        <div class="trans-con">
	            <div class="jync-banner"><img src="<%=contextPath %>/res/images/jync-top-img.jpg"></div>
	            <div class="jync-box">
	...
	<div class="bg account-box" style="display: none;">
	    <div class="account position-middle">
	        <div class="account-con">
	            <div class="header"><img src="<%=contextPath %>/res/images/banner01.gif" /></div>
	            <div class="section01">
	                <h1 class="title"><img src="<%=contextPath %>/res/images/sec01h.png" /></h1>
	                <ul class="sec01-main">
	                    <li><p><img src="<%=contextPath %>/res/images/yuyue.png" /><b>第一时间&nbsp;在线预约</b></p><span>在线预约账户诊断，随时随地。</span></li>
	                    <li><p><img src="<%=contextPath %>/res/images/huifang.png" /><b>第二时间&nbsp;客户回访</b></p><span>专属助理回访，确认您的交易情况，安全可靠。</span></li>
	                    <li class="nobor"><p><img src="<%=contextPath %>/res/images/zhenduan.png"/><b>第一时间&nbsp;在线预约</b></p><span>分析师对您的账户进行深入分析，出具书面分析报告。</span></li>
	                </ul>
	            </div>
	            <div class="section02">
	                <h1 class="title"><img src="<%=contextPath %>/res/images/sec02h.png" /></h1>
	                <ul class="sec02-main">
...
<script type="text/javascript">
	...
	$(function(){
		...
		viewSocket.onmessage = function(event) {
			...
			if(deleteFlag){
				...
				if(replyCount > 99){
					$('#interaction_count_id').html('99+');
				}else{
					$('#interaction_count_id').html(replyCount);
				}
			}else{
				var viewInfo = {
					...
					adviceId : data.adviceId == 1?'策略':'操作建议',
					mineralId : data.mineralId == 1?'油':'金属',
					id : data.id,
					teacherId : data.teacherId,
					teacherName : data.teacherName,
					teacherIconId : data.teacherIconId
				}
				var node = new viewCustomerDom(viewInfo).str2node();
		...
		actualSocket.onmessage = function(event) {
			var data = event.data;
			data= $.parseJSON(data);
			$('#actual_li').addClass("active").siblings().removeClass("active");
			$('#msg_actual_div').show();
			$('#msg_interaction_div').hide();
			var msg = data.msg;
			var deleteFlag = data.deleteFlag;
			...
			if(deleteFlag){
				...
				var node = new actualCustomerChatDom({
					content : msg,
					basepath : contextPath,
		...
		interactionSocket.onerror = function(event) {
			console.log('提问讲师WebSocket错误...',event);
		};  
		interactionSocket.onopen = function(event) {
			console.log('提问讲师WebSocket连接建立');
		};  
		interactionSocket.onmessage = function(event) {
			...
			if(data.replyTime){
				//讲师回复移除上一条回复按钮
				$('#interaction_li_'+data.quesCreatorId+'_'+data.replyViewId+' .chat_container').find('div.chart-new a.ask-a').remove();
				...
				str += '<p class="function"><i>'+data.replyTime+'</i><a viewId="'+data.replyViewId+'" teacherId="'+data.teacherId+'" questionType="2" href="javascript:;" class="ask-a">回复</a></p>';
				str += '</div>';
				str += '</li>';
				str += '</ol>';
				str += '</div>';
				$('#interaction_li_'+data.quesCreatorId+'_'+data.replyViewId+' .chat_container').append(str);
				$('#teacherView_li'+data.replyViewId+' a.ask-a').remove();
				//讲师回复数量
				var replyCount = data.replyCount;
				if(replyCount > 99){
					$('#interaction_count_id').html('99+');
				...
				var node = new interactionCustomerChatDom(data).str2node();
		...
		var searchUrl = '<%=contextPath %>/front/quotation/teacherViewScreen.htm';
    ...
    $('.back-btn').on('click',function(){
    	window.location.href="<%=contextPath %>/index/frontIndex.htm";
    });
  	//关闭
    ...
    $(document).on('click','.ask-a',function(){
   		//弹出向讲师提问输入框
   		$(".q-t-layout").show();
   		$('#askToTeacher_content').focus();
   		//讲师ID
   		$("#teacher_id").val($(this).attr('teacherId'));
   		//观点ID
   		$("#view_id").val($(this).attr('viewId'));
   		//提问类型 提问、回复
   		$('#questionType').val($(this).attr('questionType'));
    });
	});
    //选择图片 actual_ta
    function getImg(){
	...
	//添加私信
	function customerToTeacher(){
		var askTeacherUrl= '<%=contextPath %>/front/quotation/addViewMessage.htm';
		//提问内容
		var content  = $('#askToTeacher_content').val();
		//观点ID
		...
		var params={content:content,viewId:viewId,questionType:questionType};
		$.ajax({
			type:"POST",
			url:askTeacherUrl,
			data: params,
			...
			success:function(data){
				if(data){
					// 隐藏提问输入框
					$(".q-layout").hide();
					$('#askToTeacher_content').val("");
					//发送私信
					var pmStr = JSON.stringify(data);
					window.interactionSocket.send(pmStr);
				}
			}
		});
	}
    ...
    function lookBigImg(fileId){
		$("#dynamic_big_img").attr("src","<%=contextPath %>/download/file/show.htm?id="+fileId);
		var image = new Image();
		image.src = "<%=contextPath %>/download/file/show.htm?id="+fileId;
