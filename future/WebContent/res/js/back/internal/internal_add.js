	...
	$('.finish').on('click',function(){
		...
		if(specialArr&&specialArr.length>0){
			for (var i = 0; i < specialArr.length; i++) {
				console.log("专题："+specialArr[i].value);
		...
		//表单对象
		var $viewpointForm = $('#internalForm');
		...
		$.post(url,$viewpointForm.serialize(),function(data){
			if(!data){
				...
				if(data.success==0){
					...
					var internalType = $('#hiddenInternalType').val();
					window.location.href=contextPath+"/back/internal/toInternalListPage.htm?internalType="+internalType;
				}
			}
		},'json');
	});
