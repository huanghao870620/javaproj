...
public class BuyersServiceImpl extends BaseServiceImpl<Buyers, BuyersMapper> implements BuyersService<Buyers> {
	...
	/*@RequestParam(value = "sidPhotoFile", required = false) MultipartFile sidPhotoFile*/
	/**
	 * 修改头像
	 * @return
	 * @throws IOException 
	 ...
	 */
	public String updateHeadPortial(Long buyerId,MultipartFile headPortialFile,String sign,GoodsService<Goods> goodsService,HttpServletRequest request) throws IllegalStateException, IOException {
		JSONObject object = new JSONObject();
		if(!sign.equals(Security.getSign(new String[]{
				""
		}))){
		...
		goodsService.uploadFile(request, headPortialFile, PhotoType.GOODS_ACCORDING_TO_POSITIVE_TYPE,gatpFile); //商品正面视图
